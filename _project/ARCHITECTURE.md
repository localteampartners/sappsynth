...
- **Effects** (`source/dsp/effects`) — global post-sum chorus / echo /
  reverb, allocated in prepare only.
- **Telemetry** (`source/telemetry`) — lock-free SPSC sample tap feeding the
  Lab scope/spectrum; one atomic load when disabled.
- **Plugin adapter** (`source/plugin`) — APVTS ↔ PatchState, MIDI → events,
  state blobs incl. unit seed, vintage editor (filmstrip knobs from
  `assets/`, generated by `scripts/generate_ui_assets.py`), LabPanel.
- **Lab/offline** (`source/lab`, `tools/`) — WAV writer, FFT analyzer,
  offline renderer, `sapp-render`, `sapp-bench`, `SappUiShot` (offscreen
  editor screenshot).
